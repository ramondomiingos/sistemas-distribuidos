...
def should_publish_execute(request_id: str):
    ...
    try:
        ...
        responses = PrivacyRequestServiceService(db).get_by_privacy_request_and_operation(
            request_id, 'PREPARE_DELETE')
...
def should_finished_request(request_id: str):
    ...
    try:
        service_list = ServiceService(db).get_multi()
        responses = PrivacyRequestServiceService(db).get_by_privacy_request_and_operation(
            request_id, 'PERFORM_DELETE')
        logger.info(f"Faltam {len(service_list) - len(responses)} serviços responderem")
        return len(service_list) == len(responses)  # fix: era != (lógica invertida)
    finally:
        db.close()